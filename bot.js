...
async function sendWelcome(chatId) {
  const welcomeMessage = [
    "Hi there! Welcome to the April/May National AIESEC Membership Survey 💙",
    "",
...
const previousCodeStatus = session.answers.has_existing_nams_code;
...
previousCodeStatus !== session.answers.has_existing_nams_code;

if (roleChanged || lcChanged || codeChanged) {
...
session.index =
  session.flow.findIndex(
    (question) => question.field === currentField
  ) + 1;
```
